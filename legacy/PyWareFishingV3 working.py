...
class App(CTk):
    ...
    def build_automation_tab(self, parent):
        ...
        p_gain_var = StringVar(value="0.93")
        ...
        d_gain_var = StringVar(value="0.07")
    ...
    def build_utilities_tab(self, parent):
        ...
        # sundial_delay
        CTkLabel(auto_totem, text="Sundial Delay:").grid(row=3, column=0, padx=12, pady=10, sticky="w")
        sundial_delay_var = StringVar(value="15")
        self.vars["sundial_delay"] = sundial_delay_var
        CTkEntry(auto_totem, width=120, textvariable=logging_cycle_var).grid(row=3, column=1, padx=12, pady=10, sticky="w")
    ...
    def _pid_control(self, error, bar_center=None):
        ...
        if bar_center is not None and self.last_bar_x is not None and dt > 0:
            ...
            bar_moving_toward_target = (
                (bar_velocity > 0 and error_delta < 0) or
                (bar_velocity < 0 and error_delta > 0)
            )
            ...
            damping_multiplier = 5.0 if (
                error_magnitude_decreasing and bar_moving_toward_target
            ) else 0.2
    ...
    def _predictive_control(self, fish_x, bar_center, fish_left, fish_right, bar_left, bar_right):
        ...
        stopping_distance_multiplier = stopping_distance_multiplier * 1.7

        velocity_smoothing = velocity_smoothing * 2
    ...
    def _check_totem_trigger(self, shake_x, shake_y):
        ...
        try:
            ...
            sundial_delay = float(self.vars["sundial_delay"].get())
        except (ValueError, KeyError):
            trigger_secs = 60.0  # Safe Fallback
            sundial_delay = 15
