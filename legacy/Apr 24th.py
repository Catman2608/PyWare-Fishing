...
PyWare Fishing v3 - Terms of Use
...
class App(CTk):
    ...
    def _take_debug_screenshot(self):
        """
        Capture the configured area and save a debug image.
        """
        ...
        try:
            # cv2.imwrite syntax: cv2.imwrite(filename, img[, params])
            cv2.imwrite(os.path.join(BASE_PATH, "debug_bar.png"), img)
            if fallback == True:
    ...
    def _check_totem_trigger(self, shake_x, shake_y):
        ...
        template = self.templates.get("day_night")
        found = self._find_template(frame, template, confidence=0.7)

        if found is not None and use_sundial == "on":
            time.sleep(0.2)
