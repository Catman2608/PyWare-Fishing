...
class App(CTk):
    ...
    def _enter_minigame(self):
        ...
        # Ensure defaults exist to prevent None crashes
        charge_left2 = None
        charge_right2 = None
        charge_size2 = None
        bar_size = None
        ...
        while self.macro_running: # Main macro loop
            try:
                ...
                if restart_method == "Friend Area":
                    ...
                    # Cache only after fallback fill, so we never overwrite good values with None
                ...
                bars_found = (left_x is not None and right_x is not None 
                              and isinstance(left_x, (int, float)) and isinstance(right_x, (int, float)))
                if bars_found == True:
                    ...
                    if track_charges == "off":
                        ...
                        max_left = fish_left + right_deadzone
                        max_right = fish_right - left_deadzone
                    # Compute charge values (put in try/except loop to prevent crashes)
                    try:
                        if track_charges == "on" and bars_found:
                            charge_left2, charge_right2, charge_size2, charge_left = self._detect_charge_region(
                                frame,
                                bar_center,
                                bar_size,
                                fish_top,
