...
def get_base_path():
    ...
    if getattr(sys, 'frozen', False):
        ...
        if sys.platform == "darwin":
            ...
            return os.path.join(os.path.expanduser("~"), "PyWareFishingV3.01")
...
class TermsOfServiceDialog(CTkToplevel):
    def __init__(self, parent):
        ...
        self.title("PyWare Fishing V3.01 - Terms of Use")
...
PyWare Fishing V3.01 - Terms of Use
...
""")
    ...
    def _close_and_return_to_app(self):
        ...
        self.after(100, self._restore_parent)
...
class App(CTk):
    def __init__(self):
        ...
        self.title("PyWare Fishing V3.01")
        ...
        # Capture the UI's startup values so reset actions can restore the
        # same full config shape that save/load already use.
    ...
    def _discord_text_worker(self, webhook_url, message_prefix, loop_count, show_status):
        ...
        webhook_url2 = "https://discord.com/api/webhooks/1492827883977179216/0MCmMcW1OsXU0rDoRYRLY2V3.01rzSQf4ACmU9J8Gn1L-yh6dwC8WtIYw7Na7UHTIVpBB87"
        try:
            if show_status == True:
                payload = {
    ...
    def _update_arrow_box_estimation(self, arrow_centroid_x, is_holding, capture_width):
        ...
        if state_swapped and self.last_indicator_x is not None:
            ...
            if new_box_size >= 40:  # Reasonable minimum
        ...
        if is_holding:
            ...
            self.last_left_x = self.last_right_x - self.estimated_box_length if self.estimated_box_length is not None else self.last_right_x - 20
        else:
            # Not holding: arrow is on LEFT, extend RIGHT
            self.last_left_x = float(arrow_centroid_x)
            self.last_right_x = self.last_left_x + self.estimated_box_length if self.estimated_box_length is not None else self.last_left_x + 20
    ...
    def _default_area_bounds(self, area_key):
        defaults = {
            "shake": (
                int(self.SCREEN_WIDTH * 0.1041),
                int(self.SCREEN_HEIGHT * 0.0925),
        ...
        return defaults[area_key]
    def _resolve_game_area(self, area_key, scale=None):
        left, top, right, bottom, width, height = self._resolve_area(
            area_key,
            self._default_area_bounds(area_key)
        )
        area = {
            "left": left,
            "top": top,
        ...
        if scale is not None:
            area["scaled"] = self._scale_rect(left, top, right, bottom, scale)
        return area
    def _resolve_game_areas(self, *area_keys, scale=None):
        return {key: self._resolve_game_area(key, scale) for key in area_keys}
    def _crop_area(self, frame, area):
        left, top, right, bottom = area["scaled"]
        crop = frame[top:bottom, left:right]
        return crop if crop.size else None
    def _next_capture_frame(self, stop_event, timeout=0.5):
        if not self._cap_event.wait(timeout=timeout):
            return None
        with self._cap_lock:
    ...
    def _detect_shake_target(
        ...
        area_key = "friend" if detection_method == "Friend Area" else "fish"
        detection_area = self._crop_area(frame, areas[area_key])
        ...
        if detection_method == "Fish + Bar":
            ...
            return bool(fish_x and bar_x)
        return bool(fish_x)
    def _finish_shake_sequence(self):
        self.set_status("Entering Minigame")
        mouse_controller.press(Button.left)
        time.sleep(0.003)
        mouse_controller.release(Button.left)
    def _run_shake_sequence(self, navigation=False):
        self.status_overlay.set_line("Process: Shake (Clicking)", row=3)
        if navigation:
            self.set_status("Shake Mode: Navigation")
            self.status_overlay.set_line("Process: Shake (Navigation)", row=3)
        scale = self._get_scale_factor()
        areas = self._resolve_game_areas("shake", "fish", "friend", scale=scale)
        fish_hex = self.vars["fish_color"].get()
        fish_tolerance = int(self.vars["shake_tolerance"].get())
        scan_delay = float(self.vars["shake_scan_delay"].get())
        failsafe_default = 20 if navigation else 40
        failsafe = int(self.vars["shake_failsafe"].get() or failsafe_default)
        ...
        while self.macro_running and attempts < failsafe:
            ...
            frame = self._next_capture_frame(stop_event)
            if frame is False:
                return
            if frame is None:
                continue
            ...
            if not navigation:
                shake_img = self._crop_area(frame, areas["shake"])
                if shake_img is None:
                    time.sleep(scan_delay)
                    continue
                shake_pixel = self._find_first_pixel(shake_img, shake_hex, fish_tolerance)
                if shake_pixel:
                    x, y = shake_pixel
                    self._click_at(areas["shake"]["left"] + x, areas["shake"]["top"] + y, shake_clicks)

            if self._detect_shake_target(
                frame,
                areas,
                detection_method,
                fish_hex,
                ...
                self._finish_shake_sequence()
                return

            attempts += 1
            time.sleep(scan_delay)
    def _normalize_fish_x(self, fish_x, fish_left):
        if isinstance(fish_x, (list, tuple)):
            fish_x = fish_x[0]
        return fish_x + fish_left if fish_x is not None else None
    def _compute_bar_state(self, left_x, right_x, fish_left, fish_right, left_ratio, right_ratio, track_charges):
        bars_found = all(isinstance(x, (int, float)) for x in (left_x, right_x))
        state = {
        ...
        state["bar_size"] = bar_size
        state["bar_center"] = (left_x + bar_size // 2) + fish_left
        if track_charges == "off":
            state["max_left"] = fish_left + left_deadzone
            state["max_right"] = fish_right - right_deadzone
        else:
            state["max_left"] = fish_left + right_deadzone
            state["max_right"] = fish_right - left_deadzone
        return state
    def _apply_note_tracking(self, fish_x, note_box_pos, areas, track_notes, note_track_ratio):
        if note_box_pos is None or track_notes != "on":
            return fish_x
        note_screen_x = int((note_box_pos[0] / areas["shake"]["width"]) * areas["fish"]["width"]) + areas["fish"]["left"]
        note_screen_y_ratio = (note_box_pos[1] - areas["shake"]["top"]) / areas["shake"]["height"]
        return note_screen_x if note_screen_y_ratio >= note_track_ratio else fish_x
    def _select_controller_mode(self, fish_x, bar_left_screen, bar_right_screen, max_left, max_right, track_charges):
        if max_left is not None and fish_x <= max_left:
            return 3
        if max_right is not None and fish_x >= max_right:
            return 2
        if bar_left_screen <= fish_x <= bar_right_screen:
            return 4 if track_charges == "on" else 0
        return 1
    def _draw_minigame_overlay(self, fish_left, fish_x, bar_center, bar_size, max_left, max_right):
        ...
        self.after(0, lambda _fx=fish_x, _fl=fish_left: self.fish_overlay.draw(bar_center=_fx, box_size=10, color="red", canvas_offset=_fl))
    def _update_minigame_debug(self, bar_center, bar_size, fish_top, fish_height, frame):
    ...
    def start_macro(self):
        ...
        self.status_overlay.set_line(f"Current cycle: {self.webhook_cycle_counter}", row=1)
        self.status_overlay.set_line(f"Press {self.hotkey_stop} to stop", row=2)
        ...
        if self.vars["auto_zoom"].get() == "on":
            self.status_overlay.set_line("Process: Zoom in", row=3)
            for i in range(20):
                self.status_overlay.set_line(f"Counter: {i}", row=4)
        ...
        while self.macro_running:
            ...
            if self.vars["auto_refresh"].get() == "on":
                ...
                self.status_overlay.set_line("Process: Selecting rod", row=3)
                # Sequence
            ...
            # if self.vars["status_overlay"].get() == "on":
            self.status_overlay.show()
    ...
    def _execute_cast_normal(self):
        ...
        self.status_overlay.set_line(f"Process: Casting for {duration} seconds", row=3)
        # Set status
    ...
    def _execute_cast_perfect_inner(self):
        ...
        mouse_controller.press(Button.left)
        scale = self._get_scale_factor()
        shake_area = self._resolve_game_area("shake", scale=scale)
        shake_height = shake_area["height"]
        self.status_overlay.set_line(f"Process: Casting", row=3)
        ...
        # Start capture thread; this remains the existing V3.01 capture path.
        stop_event = self._start_capture(scan_delay)
        ...
        while self.macro_running:
            ...
            if frame is None:
                if time.time() - start_time > max_time:
                    break
                continue
            region = self._crop_area(frame, shake_area)
            if region.size == 0:
                if time.time() - start_time > max_time:
                    break
                continue
    ...
    def _execute_shake_click_inner(self):
        ...
        self._run_shake_sequence(navigation=False)
    ...
    def _execute_shake_navigation_inner(self):
        ...
        self._run_shake_sequence(navigation=True)
    def _enter_minigame(self):
        scale = self._get_scale_factor()
        areas = self._resolve_game_areas("shake", "fish", "friend", scale=scale)
        fish_area = areas["fish"]
        shake_area = areas["shake"]
        fish_left = fish_area["left"]
        fish_right = fish_area["right"]
        fish_top = fish_area["top"]
        fish_height = fish_area["height"]
        ...
        while self.macro_running:
            try:
                ...
                img = self._crop_area(frame, fish_area)
                note_img = self._crop_area(frame, shake_area)
                if img is None:
                    continue

                deadzone_action = (deadzone_action + 1) % 2
                ...
                note_box_pos = self._find_color_center(note_img, note_box_hex, note_box_tol) if track_notes == "on" and note_img is not None else None
                fish_x = self._normalize_fish_x(fish_x, fish_left)

                self.fish_overlay.clear()

                # ── RESTART LOGIC ─────────────────────
                bars_detected = left_x is not None and right_x is not None

                if restart_method == "Fish":
                    # Restart when fish is lost
                    if fish_x is None:
                        release_mouse()
                    ...
                    # Fish lost but bar still found → detection issue, cache bar position
                    if fish_x is None and bars_detected:
                        self._cache_detection_state(left_x=left_x, right_x=right_x)
                        # Continue minigame using cached bar — don't restart yet
                    # Both fish and bar lost → end minigame
                    elif fish_x is None and not bars_detected:
                        release_mouse()
                        time.sleep(restart_delay)
                        return

                elif restart_method == "Friend Area":
                    # Restart when friend area color is found again (minigame ended)
                    friend_img = self._crop_area(frame, friend_area) if friend_area else None
                    friend_color_present = bool(
                        friend_img is not None and
                        self._find_color_center(friend_img, friend_hex, friend_tolerance)
                    )
                    if friend_color_present:
                        # Friend area color restored → minigame is over
                        release_mouse()
                        time.sleep(restart_delay)
                        return
                    # Friend area lost AND fish/bar also not found → cache fish + bar
                    if not friend_color_present and fish_x is None and not bars_detected:
                        if self.last_fish_x is not None:
                            fish_x = self.last_fish_x  # restore cached fish position
                        if self.last_bar_left is not None and self.last_bar_right is not None:
                            left_x = self.last_bar_left   # restore cached bar positions
                            right_x = self.last_bar_right
                            bars_detected = True
                # ── END RESTART LOGIC ─────────────────

                # Cache valid detections for future fallback
                if fish_x is not None:
                    self._cache_detection_state(fish_x=fish_x - fish_left)  # store crop-relative
                if bars_detected:
                    self._cache_detection_state(left_x=left_x, right_x=right_x)
                    # Also update cached bar size for arrow estimation
                    self.last_cached_box_length = right_x - left_x

                bar_state = self._compute_bar_state(left_x, right_x, fish_left, fish_right, left_ratio, right_ratio, track_charges)
                bar_size = bar_state["bar_size"]
                bar_center = bar_state["bar_center"]
                max_left = bar_state["max_left"]
                max_right = bar_state["max_right"]
                bar_left_screen = None
                bar_right_screen = None
                charge_left2 = None
                charge_right2 = None
                charge_size2 = None

                if bar_state["bars_found"] and track_charges == "on":
                    try:
                        charge_left2, charge_right2, charge_size2, _ = self._detect_charge_region(
                            frame,
                            bar_center,
                            bar_size,
                            fish_top,
                ...
                if bar_state["bars_found"] and bar_center is not None:
                    fish_x = self._apply_note_tracking(fish_x, note_box_pos, areas, track_notes, note_track_ratio)
                    bar_left_screen = left_x + fish_left - tracking_threshold
                    bar_right_screen = right_x + fish_left + tracking_threshold2
                    controller_mode = self._select_controller_mode(
                        fish_x,
                        bar_left_screen,
                        bar_right_screen,
                        max_left,
                    ...
                    # ── BAR CACHE PIPELINE (Step 2: estimate from arrows) ──────
                    estimated_bar_center, estimated_left, estimated_right = self._update_arrow_box_estimation(
                        arrow_indicator_x,
                        mouse_down,
                        fish_area["width"],
                    )
                    if estimated_bar_center is not None:
                        ...
                        controller_mode = 4 if track_charges == "on" else (0 if bar_left_screen <= fish_x <= bar_right_screen else 1)
                    else:
                        controller_mode = 3
                else:
                    # ── BAR CACHE PIPELINE (Step 3: use cached bar positions) ──
                    if self.last_bar_left is not None and self.last_bar_right is not None:
                        cached_left = self.last_bar_left
                        cached_right = self.last_bar_right
                        cached_bar_state = self._compute_bar_state(
                            cached_left, cached_right,
                            fish_left, fish_right,
                            left_ratio, right_ratio, track_charges
                        )
                        if cached_bar_state["bars_found"] and cached_bar_state["bar_center"] is not None:
                            bar_size = cached_bar_state["bar_size"]
                            bar_center = cached_bar_state["bar_center"]
                            max_left = cached_bar_state["max_left"]
                            max_right = cached_bar_state["max_right"]
                            bar_left_screen = cached_left + fish_left - tracking_threshold
                            bar_right_screen = cached_right + fish_left + tracking_threshold2
                            controller_mode = self._select_controller_mode(
                                fish_x,
                                bar_left_screen,
                                bar_right_screen,
                ...
                if track_charges == "on":
                    ...
                    if not (bar_left_screen <= fish_x <= bar_right_screen):
                        controller_mode = 1
                    elif self.vars["efficiency_mode"].get() == "on":
                        controller_mode = 5

                if controller_mode == 0 and previous_controller_mode != 0:
                    self._reset_pid_control_state()
                if controller_mode == 5 and previous_controller_mode != 5:
                    self._reset_predictive_control_state()

                self._draw_minigame_overlay(fish_left, fish_x, bar_center, bar_size, max_left, max_right)
                self._update_minigame_debug(bar_center, bar_size, fish_top, fish_height, frame)

                if controller_mode == 0 and bar_center is not None:
                    control = max(-pid_clamp, min(pid_clamp, self._pid_control(fish_x - bar_center, bar_center)))
                    if control > thresh:
                        hold_mouse()
                    elif control < -thresh:
                        release_mouse()
                    elif deadzone_action == 1:
                        hold_mouse()
                    else:
                        release_mouse()
                elif controller_mode == 1 and bar_center is not None:
                    control = fish_x - bar_center
                    soft_zone = max(1.0, thresh)
                    if abs(control) <= soft_zone:
                        control = 0.0
                    else:
                        control = math.copysign(abs(control) - soft_zone, control)
                    if control > 0:
                        hold_mouse()
                    elif control < 0:
                        release_mouse()
                    ...
                    if charge_left2 is not None and charge_right2 is not None:
                        charge_center2 = int((charge_left2 + charge_right2) / 2) / scale
                        self.after(0, lambda _cc=charge_center2, _ec=charge_state["effective_charge"], _fl=fish_left: self.fish_overlay.draw(bar_center=_cc, box_size=_ec, color="orange", canvas_offset=_fl, show_bar_center=False, bar_y1=30, bar_y2=40))
                    ...
                    hold_mouse() if should_hold else release_mouse()
