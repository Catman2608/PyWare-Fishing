...
# Terms Of Service Dialogue - Fixed for macOS (use tk.Toplevel instead of CTkToplevel)
class TermsOfServiceDialog(tk.Toplevel):
    def __init__(self, parent=None):
        ...
        # Pages
        self.page_tos = tk.Frame(self.container)
        ...
        self.back_btn = tk.Button(nav_bar, text="Back", command=self.go_back)
        self.next_btn = tk.Button(nav_bar, text="Next", command=self.go_next)
        self.finish_btn = tk.Button(nav_bar, text="Finish", command=self.finish)
    ...
    def build_tos_page(self, parent):
        ...
        text_frame.grid(row=0, column=0, padx=12, pady=10, sticky="nsew")
        text_frame.grid_rowconfigure(0, weight=1)
        text_frame.grid_columnconfigure(0, weight=1)

        textbox = tk.Text(text_frame, wrap="word", relief="sunken", borderwidth=1, height=20)
        scrollbar = tk.Scrollbar(text_frame, orient="vertical", command=textbox.yview)
        textbox.configure(yscrollcommand=scrollbar.set)
        
        textbox.grid(row=0, column=0, sticky="nsew")
        scrollbar.grid(row=0, column=1, sticky="ns")

        textbox.insert("1.0", """PyWare Fishing V3.1 - Terms of Use
...
🚀 Thank you for using PyWare Fishing! 🚀""")
    ...
    def build_setup_page(self, parent):
        ...
        textbox.insert("1.0", """Step 1: Download and extract the config pack and images pack from https://drive.google.com/drive/folders/1e9tZwDtAaiYKTVFeArjWTIuztLgLg88a?usp=drive_link
...
class App(CTk):
    def __init__(self):
        ...
        if first_launch or not state.get("tos_accepted", False):
            ...
            # Make main window invisible (NOT withdrawn)
            self.attributes('-alpha', 0.0)

            # Ensure Tk is fully initialized before dialog
            self.update_idletasks()
            ...
            # Restore visibility BEFORE any destroy logic
            self.attributes('-alpha', 1.0)

            # Safety: check attribute exists
            accepted = getattr(dialog, "accepted", False)

            if not accepted:
                # Delay destroy to avoid native crash
        ...
        while self.macro_running:
            try:
                ...
                if detection_source == 0:
                    ...
                    # Completed: New: Cache Real Box Size
