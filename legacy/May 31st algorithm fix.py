...
import math
import subprocess
#Loggingimport requests
import io
#Initialize Controllerskeyboard_controller = KeyboardController()
...
if sys.platform == "win32":
    ...
    def _get_hwnd(window):
        ...
        candidates = (
            native,
            getattr(native, "Handle", None),              getattr(native, "Handle", None),  #WinForms BrowserForm -> System.IntPtr            getattr(window, "Handle", None),
    ...
    def make_window_translucent(window, transparency):
        ...
        user32.SetWindowLongW(hwnd, GWL_EXSTYLE, current_style | WS_EX_LAYERED)
                #Range Is 0 (fully Transparent) To 255 (fully Opaque).        opacity_alpha = int(255 * transparency)
...
def get_base_path():
    ...
    if getattr(sys, 'frozen', False):
        compiled = True
                #Compiled App → Use User Directory        if sys.platform == "darwin":
            return os.path.join(
    ...
    compiled = False
        #Dev Mode → Project Directory    return os.path.dirname(os.path.abspath(__file__)), compiled

BASE_PATH, IS_COMPILED = get_base_path()
#CONFIG FOLDERCONFIGS_FOLDER = os.path.join(BASE_PATH, "configs")
LAST_CONFIG_FILE = os.path.join(BASE_PATH, "last_config.json")
UI_PATH = os.path.join(BASE_PATH, "ui")
os.makedirs(CONFIGS_FOLDER, exist_ok=True)
#Open Base Folderdef open_base_folder():
    folder = BASE_PATH
    if sys.platform == "win32":
        os.startfile(folder)
    elif sys.platform == "darwin":      elif sys.platform == "darwin":  #Macos        subprocess.run(["open", folder])
    else:      else:  #Linux        subprocess.run(["xdg-open", folder])
def cgimage_to_srgb_numpy(image):
    width = Quartz.CGImageGetWidth(image)
    height = Quartz.CGImageGetHeight(image)
    bytes_per_row = width * 4
        #Create SRGB Color Space    color_space = Quartz.CGColorSpaceCreateWithName(
        Quartz.kCGColorSpaceSRGB
    )
        #Allocate Buffer    raw = np.empty((height, width, 4), dtype=np.uint8)
        #Create Bitmap Context Targeting Numpy Buffer    context = Quartz.CGBitmapContextCreate(
        raw,
        width,
        height,
        8,
        ...
        #Draw Image Into SRGB Context    Quartz.CGContextDrawImage(
        context,
        Quartz.CGRectMake(0, 0, width, height),
        image
    )
        #RGBA -> BGR    bgr = raw[:, :, :3][:, :, ::-1]
    return bgr.copy()
#Screen Dimensions Via Mss — Use Monitor[1] (primary) Not Monitor[0] (virtual Combined).#On Windows With DPI Scaling, Pywebview's X/y/width/height Use Physical Pixels,#So We Must Query The Raw Physical Resolution, Not The Scaled Logical Resolution.with mss.mss() as _sct:
    if len(_sct.monitors) > 1:
        _m = _sct.monitors[1]           _m = _sct.monitors[1]   #Primary Monitor    else:
        _m = _sct.monitors[0]           _m = _sct.monitors[0]   #Fallback: Only One Entry Exists    SCREEN_WIDTH  = _m["width"]
    SCREEN_HEIGHT = _m["height"]
    SCREEN_LEFT   = _m["left"]
    SCREEN_TOP    = _m["top"]
#On Windows, Mss Reports Logical (DPI-scaled) Pixel Dimensions.#Pywebview's Create_window Width/height Also Use Logical Pixels, So They Match.#However We Must Account For The Monitor's Top-left Offset (multi-monitor Setups#Where The Primary Isn't At 0,0) When Positioning Overlay Windows.if sys.platform == "win32":
    try:
        import ctypes as _ctypes
        _ctypes.windll.shcore.SetProcessDpiAwareness(2)          _ctypes.windll.shcore.SetProcessDpiAwareness(2)  #Per-monitor DPI Aware    except Exception:
        pass
...
APP_VERSION = "4.0"
#Area Selector — Pywebview-based (no Tkinter)class AreaSelector:
    """
    ...
    """
        #Path To The Overlay HTML Relative To The Ui/ Folder    HTML_FILE = os.path.join(UI_PATH, "area_selector.html")
    def __init__(self, parent, shake_area, fish_area, friend_area, totem_area, callback):
        self.parent   = parent
        self.callback = callback
        self._open    = True
                #Actual Window Origin Reported By JS After The Window Is Placed.                #Defaults To SCREEN_LEFT/TOP; Overwritten By Window_ready() Once JS Fires.                #This Corrects For MacOS Menu-bar Push-down And Any Other OS Chrome Offset.        self._win_origin_x = SCREEN_LEFT
        self._win_origin_y = SCREEN_TOP
                #Store Areas As Plain Dicts (width/height Keys)        self._areas = {
            "shake":  shake_area.copy(),
            "fish":   fish_area.copy(),
            "friend": friend_area.copy(),
            "totem":  totem_area.copy(),
        }
                #Create A Second, Frameless, Transparent, Fullscreen Pywebview Window.                #Js_api=self Exposes Get_areas / On_mouse_move / Save_areas To JS.                #NOTE: X/y Must Be The Primary Monitor's Actual Top-left Offset (SCREEN_LEFT/TOP).                #On Single-monitor Setups This Is Always 0,0. On Multi-monitor Setups Where The                #Primary Display Isn't The Leftmost One, SCREEN_LEFT/TOP Will Be Non-zero And The                #Window Must Be Placed There To Sit Over The Correct Screen.                #Do NOT Call Make_window_translucent() Here. That API Applies LWA_ALPHA To The                #Whole HWND, Which Fights With Transparent=True (per-pixel Alpha From The HTML).                #The HTML Canvas Already Draws Its Own Semi-transparent Dark Overlay, So The                #Pywebview Window Itself Should Stay Fully Opaque At The OS Level.        self._win = webview.create_window(
            "Area Selector",
            self.HTML_FILE,
            js_api=self,
                        #Window Style            transparent=True,
            frameless=True,
            easy_drag=False,
                        #Keep Above Everything            on_top=True,
                        #Prevent Resizing / Moving            resizable=False,
                        #Fullscreen Size — Matches The Primary Monitor Exactly            width=SCREEN_WIDTH,
            height=SCREEN_HEIGHT,
                        #Position At The Primary Monitor's Actual Origin (handles Non-zero Offsets)            x=SCREEN_LEFT,
            y=SCREEN_TOP,
    ...
    def on_mouse_move(self, mouse_x, mouse_y, current_boxes):
        ...
        if not self._open:
            return
                #Keep Python's Cached Areas Completely In Sync With Live Dragging In JS.                #JS Sends W/h Keys; Python Stores Width/height Keys — Normalise Here.        for name in ("shake", "fish", "friend", "totem"):
            ...
            if b:
                ...
                #Convert Canvas-relative Coords To Screen-absolute For Ratio Display        abs_x = mouse_x + self._win_origin_x
    ...
    def close(self):
        ...
        if self._open:
                        #_areas Are Screen-absolute; Save_areas() Expects Canvas-relative (it Adds                        #SCREEN_LEFT/TOP Back), So Subtract The Offset Here Before Passing Through.            self.save_areas({
                name: {
                    "x": b["x"] - self._win_origin_x,
...
##Eyedropper Class#class Eyedropper:
    ...
    def __init__(self, parent):
        ...
        self.last_picked_color = None
                #Create Fullscreen Transparent Pywebview Window        self._win = webview.create_window(
            "Eyedropper",
            self.HTML_FILE,
            js_api=self,
            transparent=True,
        ...
        if frame is None or frame.size == 0:
            return "#000000"
                #MSS Returns BGRA, We Have BGR After Processing        b = int(frame[0, 0, 0])
        g = int(frame[0, 0, 1])
        r = int(frame[0, 0, 2])
    ...
    def show(self):
        ...
        if self._win:
            try:
                ...
                pass
                        #Re-assert On_top Z-order After Un-hiding Without Stealing Keyboard Focus.                        #Hide()/show() On Edgechromium Does Not Guarantee The Window Is Re-stacked                        #Above The Game; Nudging Move() Forces The Compositor To Re-evaluate Z-order.            try:
...
class Api:
    ...
    def __init__(self):
        self.vars = {}         self.vars = {} #Save Entry Variables Here        self.current_config = self.get_last_config()
        self.load_settings_into_vars(self.current_config)
                #Key Hit Detection (press + Release = One Hit, Prevents Repeat-key Misfires)        self._keys_held = set()
                #Start Hotkey Listener        try:
            self.key_listener = KeyListener(on_press=self.on_key_press, on_release=self.on_key_release)
            self.key_listener.daemon = True
            self.key_listener.start()
        except Exception as e:
            print("Error: ", e)
                #Store Screen Width And Height To Use Later        self.SCREEN_WIDTH = SCREEN_WIDTH
        ...
        self.SCREEN_SCALE = ((self.SCREEN_WIDTH / 1920) + (self.SCREEN_HEIGHT / 1080)) / 2
                #Macro State        self.macro_running = False
        self.macro_thread = None
                #Detection Variables        self.last_fish_x = None
        ...
        self._pid_filtered_d = 0.0          self._pid_filtered_d = 0.0  #Used For Derivative Smoothing                #Arrow-Based Box Estimation Variables        self.last_indicator_x = None
        ...
        self.current_rod_name = "Basic Rod"
                #Calculate Scaling Factors        self.scale_x_1440 = self.SCREEN_WIDTH / 2560
        self.scale_y_1440 = self.SCREEN_HEIGHT / 1440
                #Screen Capture Variables — Mss Instances Are Per-Thread (See _Thread_Local)        self._thread_local = threading.local()
        self._monitor = {}              self._monitor = {}      #Pre-Allocated Monitor Dict, Reused Every Grab        self._scale_cache = None          self._scale_cache = None  #Cached Dpi Scale Factor                #Buffer For Capture/Logic Thread Decoupling (Used In Start_fishing())        self._cap_lock = threading.Lock()
        self._cap_frame = None            self._cap_frame = None    #Latest Full Screen Frame        self._cap_event = threading.Event()          self._cap_event = threading.Event()  #Signals A New Frame Pair Is Ready        self._active_capture_stop = None          self._active_capture_stop = None  #Stop Event For The Currently Running Capture Thread        self._active_capture_thread = None          self._active_capture_thread = None  #Thread Object For The Currently Running Capture Thread        self.webhook_cycle_counter = 0
    ...
    def start_eyedropper(self):
                #Toggle Off If Already Open        if hasattr(self, "eyedropper") and self.eyedropper and self.eyedropper.is_open():
            self.eyedropper.close()
            return
                #Create And Show Eyedropper        self.eyedropper = Eyedropper(parent=self)
        self.set_status("Eyedropper opened • Hover to preview • Click to pick • Esc to cancel")
        #Save Config    def _get_prompt_defaults(self):
        defaults = {}
    ...
    def save_config(self, config_name, settings):
        ...
        #Load Config    def load_config(self, config_name):
        try:
            if not config_name:
                return {
        ...
        #List Configs    def list_configs(self):
        try:
            configs = []
            ...
            return []
        #Settings State    def update_settings(self, settings):
        self.vars.update(settings)
    ...
    def get_startup_config(self):
        ...
        return result
        #Delete Config    def delete_config(self, config_name):
        try:
            folder = os.path.join(
                CONFIGS_FOLDER,
                config_name
    ...
    def load_misc_settings(self):
        """Load miscellaneous settings from last_config.json."""
                #Defaults        self.current_rod_name = "Basic Rod"
        self.bar_areas = {
            "shake": None,
            "fish": None,
            "friend": None,
        ...
        try:
            ...
            with open(path, "r") as f:
                data = json.load(f)
                        #Rod            self.current_rod_name = data.get("last_rod", "Basic Rod")
                        #Bar Areas            loaded_areas = data.get("bar_areas", {})
    ...
    def save_misc_settings(self):
        ...
        for key in ["shake", "fish", "friend", "totem"]:
            ...
            if isinstance(area, dict):
                ...
                #Save        data["last_rod"] = self.current_rod_name
        data["bar_areas"] = clean_bar_areas
                #Optional Hotkeys                #Data["start_key"] = ...                #Data["change_bar_areas_key"] = ...                #Data["stop_key"] = ...        with open(path, "w") as f:
            json.dump(data, f, indent=4)
            ...
            if os.path.exists(config_path):
                with open(config_path, "r") as f:
                    existing_config = json.load(f)
                        #Full Defaults            default_settings = self.get_default_settings()
                        #Preserve Colors            for color_key in self.get_default_colors().keys():
    ...
    def reset_colors(self, config_name):
        try:
            ...
            if os.path.exists(config_path):
                ...
                config_data = {}
                        #Reset Only Colors            config_data.update(
        ...
        #Area Selector    def _get_scale_factor(self):
        """
        ...
        if sys.platform == "darwin":
            try:
                ...
                scale = pixel_width / logical_width if logical_width else 1.0
                                #Normalize Tiny Floating-point Errors                if abs(scale - 1.0) < 0.15:
                    scale = 1.0
    ...
    def open_area_selector(self):
                #Toggle Off If Already Open        if hasattr(self, "area_selector") and self.area_selector and self.area_selector.is_open():
            self.area_selector.close()
            return

        screen_w = SCREEN_WIDTH
        screen_h = SCREEN_HEIGHT
                #Default Fallback Areas        def default_shake_area():
        ...
        def default_totem_area():
            ...
            return {"x": left, "y": top,
                    ...
                    "width": right - left, "height": bottom - top}
                #Load Saved Areas Or Fallback        shake_area  = self.bar_areas.get("shake")  if isinstance(self.bar_areas.get("shake"),  dict) else default_shake_area()
        ...
        totem_area  = self.bar_areas.get("totem")  if isinstance(self.bar_areas.get("totem"),  dict) else default_totem_area()
                #Callback When The Selector Window Saves And Closes        def on_done(shake, fish, friend, totem):
            ...
            self.area_selector = None
                #Open The Pywebview Overlay — No Tkinter Needed, No Thread Restrictions        self.area_selector = AreaSelector(
            parent=self,
        ...
        #Macro Helper Functions        #Hold Mouse    def hold_mouse(self):
        if sys.platform == "win32":
        ...
        #Release Mouse    def release_mouse(self):
        if sys.platform == "win32":
        ...
        #Click At    def _click_at(self, x, y, click_count=1):
        if sys.platform == "win32":
                        #Move Cursor            windll.SetCursorPos(x, y)
                        #Important: Tiny Movement So Roblox Registers Input            windll.mouse_event(MOUSEEVENTF_MOVE, 0, 1, 0, 0)
            ...
            x = int(x)
            y = int(y)
                        #Move Cursor            _move_mouse(x, y)
                        #Tiny Movement (Roblox Trick)            _move_mouse(x, y + 1)
            for i in range(click_count):
            ...
            time.sleep(0.01)
                        #Jitter To Prevent Roblox From Crashing            mouse_controller.position = (x + 3, y + 3)
            mouse_controller.position = (x, y)
            mouse_controller.press(Button.left)
            time.sleep(0.04)
            mouse_controller.release(Button.left)
        #Logging-Related Functions    def _discord_text_worker(self, webhook_url, message_prefix, loop_count, show_status):
    ...
    def _debug_log_worker(self, text, loop_count, show_status=False):
        ...
        try:
                        #Use Base Path For Logs            log_dir = BASE_PATH
            os.makedirs(log_dir, exist_ok=True)
                        #Daily Log File            log_file = os.path.join(
                log_dir,
    ...
    def _auto_bug_report(self, error_text, phase="Unknown"):
        """Send a text-only crash report to the bug report webhook or save to file."""
                #Safely Get Values And Reset State        self._set_fish_overlay_mode("idle")
        ...
        timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
                #Prepare The Report Text        report_text = (
            "Auto Bug Report\n"
    ...
    def send_logging(self, text, loop_count, show_status=True):
        ...
        if not logging_mode == "File":
                        #Logging_url            webhook_url = self.vars["logging_url"].strip()
    ...
    def _grab_screen_region(self, left, top, right, bottom):
        ...
        height = bottom - top
        if sys.platform == "darwin":
            region = Quartz.CGRectMake(left, top, width, height)             region = Quartz.CGRectMake(left, top, width, height) #Get Region                        #Capture Full Screen            image = Quartz.CGWindowListCreateImage(
            ...
            frame = cgimage_to_srgb_numpy(image)
                        #Manual Crop Using Actual Coordinates            cropped = frame[top:bottom, left:right]
            return cropped.copy()
            ...
            if width <= 0 or height <= 0:
                return None
                        #Use A Local Dict Rather Than Self._monitor To Avoid Concurrent Mutation            m = {"left": left, "top": top, "width": width, "height": height}
            if not hasattr(self._thread_local, "sct"):
                self._thread_local.sct = mss.mss()
            img = self._thread_local.sct.grab(m)
                        #MSS Returns BGRA. We Convert The Memory View To A Standard Numpy Array Safely.            frame = np.array(img, dtype=np.uint8) 
                        #Slice To BGR (dropping Alpha Channel).            bgr_frame = frame[:, :, :3]
                        #Mathematical Shift Correction Safely Applied For MacOS Stability            return bgr_frame

    def _grab_screen_full(self, thread_local=None):
                #Fallback Like Grab_screen_region        if thread_local is None:
        ...
        if sys.platform == "darwin":
            ...
            frame = cgimage_to_srgb_numpy(image)
                        #Crop Manually For Coordinate Consistency            frame = frame[0:height, 0:width]
            return frame.copy()
            ...
            img = thread_local.sct.grab(m)
                        #Convert MSS Image Safely            frame = np.array(img, dtype=np.uint8)
                        #Remove Alpha Channel            bgr_frame = frame[:, :, :3]
            return bgr_frame

    def _capture_loop_full(self, stop_event, scan_delay):
        thread_local = threading.local()
                #On Macos, Mss Uses Core Graphics Which Is Slow To Call In A Tight Loop.                #Enforce A Minimum Sleep So We Don'T Saturate The Cpu And Starve The Game                #And The Pid Thread. At 20 Fps A Frame Is ~0.05 S; Floor At 0.033 S                #(~30 Fps) So We Never Spin Faster Than The Game Can Produce New Pixels.        _mac_floor = 0.033 if sys.platform == "darwin" else 0.0
        try:
    ...
    def _start_capture(self, scan_delay):
        ...
        """
                #Overlapping Capture Threads Share _Cap_Frame/_Cap_Event/_Cap_Lock And                #Will Race Each Other, Which Causes Segfaults In The Mss/Coregraphics                #Capture Path On Macos.        self._stop_active_capture()
        self._cap_frame = None
                #Ensure These Exist        if not hasattr(self, "_cap_lock"):
            self._cap_lock = threading.Lock()
        if not hasattr(self, "_cap_event"):
            self._cap_event = threading.Event()
        self._cap_event.clear()
        stop_event = threading.Event()
        self._active_capture_stop = stop_event          self._active_capture_stop = stop_event  #Track The Active Stop Event        _mac_floor = 0.033 if sys.platform == "darwin" else 0.0
        def _loop():
            try:
                thread_local = threading.local()
                while self.macro_running and not stop_event.is_set():
    ...
    def _get_areas(self, area_key):
                #Apply Scale Factor        scale = self._get_scale_factor()
    ...
    def _detect_day_or_night(self, confidence_threshold=0.7):
        ...
        def white_mask(img):
            ...
            mask = cv2.inRange(img, lower, upper)
                        #If Completely Empty, Return Early            if mask is None or mask.size == 0:
                return None
        ...
        def best_match(frame_mask, ref_mask):
            ...
            rh, rw = ref_mask.shape
                        #Ensure Valid Dimensions            if fh == 0 or fw == 0 or rh == 0 or rw == 0:
                return 0.0
                        #Resize Reference If Needed            if rh > fh or rw > fw:
                ...
                rh, rw = ref_mask.shape
                        #Final Safety Check (CRITICAL)            if rh > fh or rw > fw:
                return 0.0

            try:
                result = cv2.matchTemplate(frame_mask, ref_mask, cv2.TM_CCOEFF_NORMED)
                                #THIS Prevents Your Exact Crash                if result is None or result.size == 0:
                    return 0.0
    ...
    def _find_last_pixel(self, frame, hex, tolerance=8):
        ...
        coords = np.argwhere(mask)
        if coords.size > 0:
            y, x = coords[-1]              y, x = coords[-1]  #Changed From [0] To [-1]            return int(x), int(y)
    ...
    def _pixel_search(self, frame, target_color_hex, tolerance=8):
        ...
        if frame is None or frame.size == 0:
            return []
                #Convert Hex To Bgr        bgr_color = self._hex_to_bgr(target_color_hex)
        if bgr_color is None:
            return []
                #Create Mask For Matching Colors (Euclidean Distance)        target = np.array(bgr_color, dtype=np.int32)
        frame_int = frame.astype(np.int32)
        diff = frame_int - target
        dist = np.sqrt(np.sum(diff ** 2, axis=-1))
        mask = (dist <= tolerance).astype(np.uint8) * 255
        y_coords, x_coords = np.where(mask > 0)
                #Return As List Of (X, Y) Tuples        if len(x_coords) > 0:
    ...
    def _find_circles(self, frame):
        ...
        """
        try:
                        #Convert BGR To Grayscale            gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
                        #Scale Circle Detection Parameters Based On Resolution                        #Reference Values Are For 2560x1440 Resolution                        #Use Average Of Scale_x_1440 And Scale_y_1440 For Uniform Circle Scaling            scale_factor = (self.scale_x_1440 + self.scale_y_1440) / 2
                        #Scale Parameters Proportionally To Resolution            scaled_min_dist = int(150 * scale_factor)
            ...
            scaled_good_max_radius = int(120 * scale_factor)
                        #Hough Circle Transform With Strict Parameters For Perfect Circles Only            circles = cv2.HoughCircles(
                gray,
                cv2.HOUGH_GRADIENT,
                dp=1,                           dp=1,           #Inverse Ratio Of Accumulator Resolution                minDist=scaled_min_dist,                    minDist=scaled_min_dist,    #Increased Distance Between Circles To Avoid Overlapping Detections                param1=100,                     param1=100,     #Higher Canny Threshold For Edge Detection                param2=100,                     param2=100,     #Much Higher Accumulator Threshold - Only Perfect Circles                minRadius=scaled_min_radius,                   minRadius=scaled_min_radius,   #Larger Minimum Radius To Ignore Small False Positives                maxRadius=scaled_max_radius                   maxRadius=scaled_max_radius   #Maximum Circle Radius            )
            if circles is not None:
                circles = np.round(circles[0, :]).astype("int")
                                #Additional Filtering: Only Accept Circles With Good Radius Range For SHAKE Buttons                good_circles = []
                for (x, y, r) in circles:
                                        #SHAKE Buttons Are Typically 50-120 Pixels Radius (scaled)                    if scaled_good_min_radius <= r <= scaled_good_max_radius:
                        good_circles.append((x, y, r))
                if good_circles:
                                        #Return The Largest Good Circle (most Likely To Be SHAKE Button)                    largest_circle = max(good_circles, key=lambda c: c[2])
                    x, y, r = largest_circle
                                        #Print(f" 🔍 Circle Detected At Local ({x}, {y}) With Radius {r} (scale: {scale_factor:.3f})")                    return (int(x), int(y))
                        #Only Use Strict HoughCircles Detection - No Backup Methods To Avoid False Positives            return None
    ...
    def _find_color_center(self, frame, target_color_hex, tolerance=8):
        ...
        if frame is None:
            return None
                #Convert Color        target_bgr = np.array(self._hex_to_bgr(target_color_hex), dtype=np.int32)
                #Convert Frame For Safe Subtraction        frame_int = frame.astype(np.int32)
        tol = int(np.clip(tolerance, 0, 255))
                #Euclidean Distance Comparison        diff = frame_int - target_bgr
        mask = np.sqrt(np.sum(diff ** 2, axis=2)) <= tol
        y_coords, x_coords = np.where(mask)
        if len(x_coords) == 0:
            return None
                #Center Calculation (Vectorized Mean)        center_x = int(np.mean(x_coords))
    ...
    def _find_color_cluster(self, frame, target_color_hex, tolerance=8, min_area=10):
        ...
        """
                #Required_Fish_Pixels        if frame is None:
            return None
                #Color Mask (Vectorized Like Your Fast Version)        target_bgr = np.array(self._hex_to_bgr(target_color_hex), dtype=np.int32)
        frame_int = frame.astype(np.int32)
        tol = int(np.clip(tolerance, 0, 255))
        mask = (np.sqrt(np.sum((frame_int - target_bgr) ** 2, axis=2)) <= tol).astype(np.uint8)
        if not np.any(mask):
            return None
                #Connected Components (Cluster Detection)        num_labels, labels, stats, centroids = cv2.connectedComponentsWithStats(mask, connectivity=8)
        if num_labels <= 1:
            return None              return None  #Only Background                #Skip Label 0 (Background)        largest_label = None
        largest_area = 0
        ...
        if largest_label is None:
            return None
                #Centroid        center_x, center_y = centroids[largest_label]
    ...
    def _find_bar_edges(
        ...
        center_y = int(np.clip(h * scan_height_ratio, 0, h - 1))
                #Convert To BGR        left_bgr = np.array(self._hex_to_bgr(left_hex), dtype=np.int32)
        right_bgr = np.array(self._hex_to_bgr(right_hex), dtype=np.int32)
                #Clamp Tolerances        tol_l = int(np.clip(tolerance, 0, 255))
        tol_r = int(np.clip(tolerance2, 0, 255))
        left_edge = None
        right_edge = None
                #Scan Multiple Y Values, Starting From The Preferred Row And Moving Outward.        for offset in range(h):
            if offset == 0:
                y_values = (center_y,)
            else:
            ...
            for y in y_values:
                ...
                line = frame[y].astype(np.int32)
                                #Left Mask (Euclidean Distance)                left_diff = line - left_bgr
                left_mask = np.sqrt(np.sum(left_diff ** 2, axis=1)) <= tol_l
                                #Right Mask (Euclidean Distance)                right_diff = line - right_bgr
                ...
                if current_right is not None and right_edge is None:
                    right_edge = current_right
                                #Fast Path: Return Immediately Once A Row Contains Both Edges.                if current_left is not None and current_right is not None:
    ...
    def _find_arrow_indicator_x(self, frame, arrow_hex, tolerance, is_holding):
        ...
        indicator_x = max(xs) if is_holding else min(xs)
                #Small Jitter Filter        if self.last_indicator_x is not None:
        ...
        bar_right_x = None
            ...
            box_size = self._last_bar_box_size
                ...
                last_right = self._last_bar_right_x
                                #Determine Which Side Based On Center Comparison                arrow_on_left_side = arrow_center_x < last_center
                                #SMART VALIDATION: Check If Arrow Is Actually Near The Bar We Think It Is                                #Calculate Distances To Both Last Known Bars                dist_to_left = abs(arrow_center_x - last_left) if last_left is not None else float('inf')
                dist_to_right = abs(arrow_center_x - last_right) if last_right is not None else float('inf')
                                #Self-correction: If Arrow Is Much Closer To The Opposite Bar, We Detected Wrong Side!                                #Threshold: Arrow Should Be Within Reasonable Distance (box_size / 4) Of Expected Bar                proximity_threshold = box_size / 4
                if arrow_on_left_side:
                                        #We Think Arrow Is On LEFT, But Verify It's Actually Near Left Bar                    if dist_to_right < dist_to_left and dist_to_right < proximity_threshold:
                                                #Arrow Is Actually Closer To RIGHT Bar - We Were Wrong!                                                #Print(f"🐟 Arrow Mode: SELF-CORRECTION - Arrow At {arrow_center_x:.0f} Closer To RIGHT Bar ({dist_to_right:.0f}px) Than LEFT ({dist_to_left:.0f}px)")                        arrow_on_left_side = False                          arrow_on_left_side = False  #Flip The Decision                else:
                                        #We Think Arrow Is On RIGHT, But Verify It's Actually Near Right Bar                    if dist_to_left < dist_to_right and dist_to_left < proximity_threshold:
                                                #Arrow Is Actually Closer To LEFT Bar - We Were Wrong!                                                #Print(f"🐟 Arrow Mode: SELF-CORRECTION - Arrow At {arrow_center_x:.0f} Closer To LEFT Bar ({dist_to_left:.0f}px) Than RIGHT ({dist_to_right:.0f}px)")                        arrow_on_left_side = True                          arrow_on_left_side = True  #Flip The Decision                                #Now Apply The Corrected Decision                if arrow_on_left_side:
                                        #Arrow Is On The LEFT Side - Update Left Bar, Keep Right Bar From Memory                    bar_left_x = arrow_center_x
                    bar_right_x = self._last_bar_right_x
                    if bar_right_x is None:
                                                #If No Right Bar In Memory, Calculate From Box Size                        bar_right_x = bar_left_x + box_size
                        ...
                        pass                         pass #Print(f"🐟 Arrow Mode: Invalid - Arrow Left {bar_left_x:.0f} >= Right {bar_right_x:.0f}")                else:
                                        #Arrow Is On The RIGHT Side - Update Right Bar, Keep Left Bar From Memory                    bar_right_x = arrow_center_x
                    bar_left_x = self._last_bar_left_x
                    if bar_left_x is None:
                                                #If No Left Bar In Memory, Calculate From Box Size                        bar_left_x = bar_right_x - box_size
                                        #Validate: Ensure Left < Right                    if bar_left_x < bar_right_x:
                        self._last_bar_left_x = bar_left_x
                        self._last_bar_right_x = bar_right_x
                        bar_center = (bar_left_x + bar_right_x) / 2.0
                ...
                if box_size > 0:
                    ...
                    self._last_bar_center = last_center
                        ...
                        bar_right_x = bar_left_x + box_size
                                                #Print(f"🐟 Arrow Mode: Initial LEFT - L={bar_left_x:.0f} (arrow), R={bar_right_x:.0f} (size={box_size:.0f})")                    else:
    ...
    def on_key_press(self, key):
                #Track Which Keys Are Currently Held So On_key_release Can Confirm A                #Full Press+release Cycle (a "hit") Before Acting. This Prevents OS                #Key-repeat Events From Firing The Action Multiple Times.        self._keys_held.add(self.normalize_key(key))
    def on_key_release(self, key):
        key = self.normalize_key(key)
                #Only Act If This Release Follows A Genuine Press (i.e. A Real "hit")        if key not in self._keys_held:
            return
    ...
    def _string_to_key(self, key_string):
        key_string = key_string.strip().lower()
                #Try Special Keys        if hasattr(Key, key_string):
            return getattr(Key, key_string)
                #Fallback To Character        return key_string
    ...
    def _get_var_number(self, key, default, cast=float):
        ...
        try:
            ...
            if value is None:
                                #Compatibility Mapping For 1600plus Key Differences                if key == "perfect_cast_timing_1600_plus":
    ...
    def _hex_to_bgr(self, hex_color):
        ...
        hex_color = hex_color.lstrip('        hex_color = hex_color.lstrip('#')        if len(hex_color) == 6:
            try:
                r = int(hex_color[0:2], 16)
                g = int(hex_color[2:4], 16)
                b = int(hex_color[4:6], 16)
                return (b, g, r)                  return (b, g, r)  #Bgr Format For Opencv

            except ValueError:
                return None

        return None
        #Do Pixel/Image/Line Search    def _do_pixel_search(self, frame):
        fish_hex = self.vars["fish_color"]
        left_bar_hex = self.vars["left_color"]
        right_bar_hex = self.vars["right_color"]
        try:         try: #Handle Nonetype And Int Properly            left_tol = int(self.vars["left_tolerance"] or 8)
            ...
            right_tol = 8
            fish_tol = 1
                #MacOS Tolerance Buffer To Make Configs Cross-Compatible        if sys.platform == "darwin":
            left_tol += 2
            right_tol += 2
            fish_tol += 2
    ...
    def _do_line_search(self, frame, original_width=None):
        ...
        """
        try:
                        #Get Minimum Line Density From Settings (configurable Via GUI)            MIN_LINE_DENSITY = self._get_rod_specific_setting("fish_line_min_density", 0.8)
            BRIGHTNESS_THRESHOLD = 10              BRIGHTNESS_THRESHOLD = 10  #Minimum Brightness For Edge Pixels                        #Reference Fish Box Dimensions At 1280x720            REFERENCE_FISH_WIDTH = 517               REFERENCE_FISH_WIDTH = 517   #Fish Box Width At 720p            REFERENCE_FISH_HEIGHT = 22               REFERENCE_FISH_HEIGHT = 22   #Fish Box Height At 720p                        #Store Original Dimensions For Coordinate Scaling            original_height, original_frame_width = frame.shape[:2]
            if original_width is None:
                original_width = original_frame_width
                        #Normalize Frame To Reference Dimensions For Consistent Detection            if original_frame_width != REFERENCE_FISH_WIDTH or original_height != REFERENCE_FISH_HEIGHT:
                ...
                width_scale = 1.0
                        #Step 1: Convert To Grayscale            grayscale = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
                        #Step 2: Laplacian Edge Detection            laplacian = cv2.Laplacian(grayscale, cv2.CV_8U)
                        #Step 3: Filter Vertical Lines By Brightness Threshold And Density            height, width = laplacian.shape
                        #Vectorized Column Density Calculation            column_densities = np.sum(laplacian > BRIGHTNESS_THRESHOLD, axis=0) / height
            line_coordinates = np.where(column_densities >= MIN_LINE_DENSITY)[0].tolist()
                        #Merge Adjacent Lines Into Single Lines            if line_coordinates:
                merged_lines = []
                ...
                line_coordinates = merged_lines
                        #Scale Line Coordinates Back To Original Frame Dimensions            if width_scale != 1.0:
                line_coordinates = [int(x * width_scale) for x in line_coordinates]
                        #Initialize State Variables If Not Already Done            if not hasattr(self, '_line_state'):
                self._line_state = {
            ...
            image_center_x = original_width // 2
                        #Need At Least 2 Lines To Identify Fish And Bars            if len(line_coordinates) < 2:
                return None, None, None
                        #INITIAL RUN: Find 2 Closest Lines To Center As Target (fish) Lines            if state['is_initial_run'] or state['initial_target_gap'] is None:
                ...
                initial_target_gap = target_right_x - target_left_x
                                #Find Bars - Closest To Left Of Left Target, Closest To Right Of Right Target                left_candidates = [x for x in line_coordinates if x < target_left_x]
                ...
                right_bar_x = min(right_candidates) if right_candidates else target_right_x
                                #Update State                state['last_target_left_x'] = target_left_x
                ...
                state['is_initial_run'] = False
                                ...
                                #SUBSEQUENT RUNS: Find Pair With Gap Matching Initial_target_gap                best_gap_diff = float('inf')
                ...
                for i in range(len(line_coordinates) - 1):
                    ...
                    if gap_diff < best_gap_diff:
                        ...
                        target_right_x = curr_right
                                #If Best Gap Is More Than 4x Initial Gap, Keep Old Positions                actual_gap = target_right_x - target_left_x
                if actual_gap > state['initial_target_gap'] * 4:
                    target_left_x = state['last_target_left_x']
                    target_right_x = state['last_target_right_x']
                                #Find Bars From Non-target Lines                other_lines = [x for x in line_coordinates if x != target_left_x and x != target_right_x]
                if len(other_lines) >= 2:
                                        #Pick Closest To Last Positions                    left_bar_x = min(other_lines, key=lambda x: abs(x - state['last_left_bar_x'])) if state['last_left_bar_x'] is not None else other_lines[0]
                    ...
                    right_bar_x = min(remaining_lines, key=lambda x: abs(x - state['last_right_bar_x'])) if remaining_lines and state['last_right_bar_x'] is not None else (remaining_lines[0] if remaining_lines else state['last_right_bar_x'])
                                        ...
                                        #Only One Non-target Line - Assign To Closest Bar                    single_line = other_lines[0]
                    if state['last_left_bar_x'] is not None and state['last_right_bar_x'] is not None:
                        ...
                        right_bar_x = target_right_x
                                        ...
                                        #No Other Lines - Use Last Known Positions                    left_bar_x = state['last_left_bar_x'] if state['last_left_bar_x'] is not None else target_left_x
                    right_bar_x = state['last_right_bar_x'] if state['last_right_bar_x'] is not None else target_right_x
                                #Update State With New Values                state['last_target_left_x'] = target_left_x
                state['last_target_right_x'] = target_right_x
                state['last_left_bar_x'] = left_bar_x
                state['last_right_bar_x'] = right_bar_x
                        #Calculate Centers            fish_x = (target_left_x + target_right_x) / 2.0
            left_x = left_bar_x
            right_x = right_bar_x
            ...
            return None, None, None
        #PID Control    def _reset_pid_state(self):
        ...
        if (
            ...
            if time_delta > 0.001:
                ...
                if error_magnitude_decreasing and bar_moving_toward_target:
                                        #APPROACHING – Strong Damping To Prevent Overshoot                    d_term = -kd * 5.0 * bar_velocity
                else:
                                        #CHASING – Light Damping To Allow Fast Movement                    d_term = -kd * 0.2 * bar_velocity
                #Update State For Next Frame        self._pid_last_error      = error
        self._pid_last_target_x   = target_line_last_x
        self._pid_last_scan_time  = current_time
                #Combined And Clamped Control Signal        control_signal = p_term + d_term
    ...
    def _predictive_control(self, fish_x, bar_center, fish_left, fish_right, bar_left, bar_right):
        ...
        bar_right: Bar Right
        """
                #Init Failsafe        if not hasattr(self, "_pred_prev_fish_x"):
            self._reset_control_state()
                #Failsafe: Missing Data        if fish_x is None or bar_center is None or bar_left is None or bar_right is None:
            should_hold = False
            return should_hold
                #Read Settings        stopping_distance_multiplier = self._get_var_number("kp", 0.93)
        velocity_smoothing = self._get_var_number("kd", 0.07)
        stopping_distance_multiplier = stopping_distance_multiplier * 1.5
        MIN_DT = 1e-3
        MAX_DT = 0.1
        MAX_VEL = 3000.0
                #Time        now = time.perf_counter()
        ...
        self._pred_prev_time = now
                #Velocities        if self._pred_prev_fish_x is not None:
        ...
        raw_bar_vel  = max(min(raw_bar_vel,  MAX_VEL), -MAX_VEL)
                #Smooth Velocities Independently Then Compute Relative        self.color_check_target_velocity = (velocity_smoothing * raw_fish_vel +
        ...
        relative_velocity = self.color_check_bar_velocity - self.color_check_target_velocity          relative_velocity = self.color_check_bar_velocity - self.color_check_target_velocity  #Bar Relative To Fish (Matches Ref Macro Sign)        self._pred_prev_fish_x = fish_x
        self._pred_prev_bar_x  = bar_center
                #Nan Guard        if not np.isfinite(relative_velocity):
            should_hold = False
            return should_hold
                #Reachability (Edge Logic)        bar_width = bar_right - bar_left
        min_reachable = fish_left  + bar_width // 2
        max_reachable = fish_right - bar_width // 2
        if fish_x < min_reachable:
                        #Target Too Far Left — Bar Can't Follow, Release            should_hold = False
            return should_hold

        elif fish_x > max_reachable:
                        #Target Too Far Right — Hold To Push Bar Right            should_hold = True
            return should_hold
                #Calculate Stopping Distance Based On Relative Velocity        stopping_distance = abs(relative_velocity) * stopping_distance_multiplier
                #Error: Positive = Bar Is Right Of Fish (Same Sign Convention As Ref Macro)        error = bar_center - fish_x
                #On-Bar: Use Stopping-Distance / Counter-Thrust Logic        if error < -stopping_distance:
                        #Bar Is Left Of Fish Beyond Stopping Distance → Hold To Move Right            should_hold = True
        elif error > stopping_distance:
                        #Bar Is Right Of Fish Beyond Stopping Distance → Release To Move Left            should_hold = False
        else:
                        #Within Stopping Distance — Counter-Thrust Based On Relative Velocity            if relative_velocity > 0:
                                #Bar Moving Right Relative To Fish → Release (Apply Left Thrust)                should_hold = False
            else:
                                #Bar Moving Left Relative To Fish → Hold (Apply Right Thrust)                should_hold = True
        return should_hold
        #Start Angler (quest)    def start_angler(self):
        ...
        angler_click_y = int(dialogue_height * angler_y_ratio) + dialogue_top
                #Backpack Key        backpack_x_ratio = self.vars["backpack_x"]
        ...
        pytesseract.pytesseract.tesseract_cmd = tesseract_path
                #Main Loop        while self.macro_running:
            time.sleep(0.1)
                        #STEP 1: CLICK E → OPEN QUEST DIALOGUE            keyboard_controller.press("e")
            time.sleep(0.05)
            ...
            if angler_click_mode == "Search":
                ...
                self._click_at(angler_click_x, angler_click_y)
                        #STEP 2: OCR QUEST AREA — GET REQUIRED FISH TEXT            time.sleep(2)
            img = self._grab_screen_full()
            ...
            time.sleep(0.25)
                        #Clear Previous Search            keyboard.press("ctrl")
            keyboard.press("a")
            keyboard.release("a")
            ...
            time.sleep(0.1)
                        #Type Fish Name            for char in required_fish:
                keyboard.press(char)
                keyboard.release(char)
                time.sleep(0.05)
            time.sleep(0.5)
                        #STEP 5: LOCATE Quest_text IN QUEST AREA VIA OCR AND CLICK IT            img = self._grab_screen_full()
            ...
            for i, text_tok in enumerate(ocr_data_q["text"]):
                ...
                if tok in required_fish or required_fish in tok:
                    ...
                    qh = ocr_data_q["height"][i]
                                        #Undo The 3× Upscale To Get Back To Screen Coords                    quest_click_x = quest_left + (qx + qw // 2) // 3
        ...
        while self.macro_running:
            ...
            gray = cv2.cvtColor(enchantment, cv2.COLOR_BGR2GRAY)
                        #Upscale Image            gray = cv2.resize(gray, None, fx=3, fy=3, interpolation=cv2.INTER_CUBIC)
                        #Sharpen Contrast            gray = cv2.threshold(gray, 150, 255, cv2.THRESH_BINARY)[1]
            text = pytesseract.image_to_string(gray)
            if mutation_enchant.lower() in text.lower():
                self.stop_macro("Enchanting finished")
            time.sleep(4)
        #Start Appraisal    def start_appraisal(self):
        self.macro_running = True
        dialogue_left, dialogue_top, dialogue_right, dialogue_bottom, dialogue_width, dialogue_height = self._get_areas("shake")
                ...
                if appraisal_mode == "Search":
                    ...
                    try:
                        if dialogue is None:
                            continue
                        dialogue_x, dialogue_y = dialogue
                                                #Convert Cropped Coordinates Back To Screen Coordinates                        screen_x = dialogue_left + dialogue_x
        ...
        #Start Fishing    def start_fishing(self):
        self.macro_running = True
        ...
        if auto_zoom == "on":
            ...
            time.sleep(0.1)
        while self.macro_running:
                        #Misc / Utilities                        #Select Rod            if auto_refresh == "on":
                ...
                self.set_status("Selecting rod")
                                #Sequence                time.sleep(bag_delay * 1.5)
                ...
                time.sleep(0.2)
                        #Logging            self._check_logging_trigger()
                        #Totem            self._check_totem_trigger(shake_x, shake_y)
            if self.vars["auto_reconnect"] == "on":
                self._auto_reconnect(shake_x, shake_y)
                        #Cast            if casting_mode == "perfect" or casting_mode == "Perfect":
                self._execute_cast_perfect()
            else:
                self.execute_cast_normal()
                        #Shake            if shake_mode == "navigation" or shake_mode == "Navigation":
        ...
        #Utilities    def _check_logging_trigger(self):
        ...
        if cd_mode == "Disabled":
            return              return  #Webhook Type Is Disabled; Do Nothing

        try:
            trigger_every = int(self.vars["logging_cycle"])
        except (ValueError, KeyError):
            trigger_every = 3              trigger_every = 3  #Safe Fallback        try:
            trigger_secs = float(self.vars["logging_time"])
        except (ValueError, KeyError):
            trigger_secs = 60.0              trigger_secs = 60.0  #Safe Fallback        if cd_mode == "Cycles":
            ...
            self.webhook_cycle_counter += 1              self.webhook_cycle_counter += 1  #Still Count Cycles For The Message Label            elapsed = time.time() - self.webhook_start_time
            if trigger_secs > 0 and elapsed >= trigger_secs:
                ...
                self.send_logging("**Time Checkpoint**", label, show_status=False)
                                #Reset The Timer So It Fires Again After Another Trigger_secs Seconds                self.webhook_start_time = time.time()
    def _check_totem_trigger(self, shake_x, shake_y):
        ...
        mode = self.vars["auto_totem_mode"]
                #Self.SCREEN_SCALE        if mode == "Disabled":
            return
        ...
        try:
            ...
            trigger_secs = 60.0              trigger_secs = 60.0  #Safe Fallback                #Cycles Mode        if mode == "Cycles":
            self.totem_cycle_counter += 1
            if not (trigger_every > 0 and self.totem_cycle_counter % trigger_every == 0):
                return
                #Time Mode        elif mode == "Time":
            elapsed = time.time() - self.totem_start_time
            if not (trigger_secs > 0 and elapsed >= trigger_secs):
                return
                        #Reset Timer Before Execution So It Starts Fresh Regardless Of Success/Failure            self.totem_start_time = time.time()
            self.totem_cycle_counter += 1
        else:
            return
                #Execute Totem        self.set_status("Using Totem")
        sundial_slot = str(self.vars["sundial_slot"])
        target_slot  = str(self.vars["target_slot"])
        rod_slot  = str(self.vars["rod_slot"])
        sundial_delay  = int(self.vars["sundial_delay"])
        desired_time = self.vars["use_sundial_mode_when"]          desired_time = self.vars["use_sundial_mode_when"]  #"Day", "Night", Or Maybe "Disabled"        totem_success = False
        confidence_threshold = 0.6
                #Detect Day / Night        current_time, best_conf = self._detect_day_or_night(confidence_threshold)
        if current_time is None:
            return              return  #Below Confidence Threshold — Skip This Cycle                #Decide Whether To Use Sundial        use_sundial = (
            desired_time in ["Day", "Night"] and
            current_time != desired_time
        )
                #Use Sundial (If Needed)        if use_sundial:
            time.sleep(0.2)
            ...
            self._click_at(shake_x, shake_y)
                        #Wait For Time Transition            time.sleep(sundial_delay)
                #Use Target Totem        time.sleep(0.2)
    ...
    def _auto_reconnect(self, center_x, center_y):
        ...
        mirror_click_x = int(shake_width * mirror_ratio) + shake_left_s
                #0.59        mirror_click_y = int(shake_height * mirror_ratio2) + shake_top_s
                #1520        reconnect_threshold = int((reconnect_threshold / 1500) * shake_width)
        ...
        while self.macro_running:
            ...
            return
        #Casting    def _execute_cast_perfect(self):
        """
        ...
        """
                #Hold Mouse        mouse_controller.press(Button.left)
                #Get Areas (Scale Factor Applied Inside _Get_Areas)        shake_left_s, shake_top_s, shake_right_s, shake_bottom_s, _, shake_height = self._get_areas("shake")
        self._fish_overlay_cast_bounds = None
        self._set_fish_overlay_mode("casting")
                #Config        white_color = self.vars.get("white_cast_color", self.vars.get("perfect_color2", "#d4d3ca"))
        ...
        target_white = np.array(self._hex_to_bgr(white_color), dtype=np.int32)
                #Resolution Scaling: Velocity Bands Are Tuned At 1440p Height        scaling_factor = self.SCREEN_HEIGHT / 1440.0
        tracking_mode = False
        green_left_x = None
        green_right_x = None
        green_y = None
        green_padding = 50
                #Velocity Tracking — Up To 5 Samples For Linear Regression        white_positions = []            white_positions = []    #(x, Y) In Region-relative Coords        white_timestamps = []           white_timestamps = []   #Parallel Perf_counter Values        MAX_VELOCITY_SAMPLES = 5
        ...
        def reset_tracking():
            ...
            white_timestamps.clear()
                #Start Capture Thread; This Remains The Existing V3.42 Capture Path.        stop_event = self._start_capture(scan_delay)
        start_time = time.time()
        time.sleep(delay_before_casting)
                #Perfect Cast Loop        while self.macro_running:
            ...
            if self._is_fish_overlay_enabled():
                ...
                self.fish_overlay.draw(
                    ...
                    bar_y2=min(1.0, white_ratio + bar_height / 2)
                        ...
                        #--- Velocity Tracking ---            now_pc = time.perf_counter()
            white_positions.append((0, white_y_top))               white_positions.append((0, white_y_top))   #X Is Irrelevant; Track Y Only            white_timestamps.append(now_pc)
            ...
            self.set_status(f"White Y: {white_y_top}")
                        #Local_distance: Pixels Remaining Until White Reaches Green            local_distance = current_distance              local_distance = current_distance  #White_y_top - Green_y; Positive = White Below Green                        #--- Velocity-band Predictive Release ---            released = False
                ...
                if velocity_y is not None and abs(velocity_y) > min_speed:
                    ...
                    if moving_toward_green and local_distance > 0:
                        time_to_impact = local_distance / abs(velocity_y)
                                                #Bounce/miss Detection: If TtI Suddenly Grows When Very Close, We Passed Green                        bounce_threshold = 40 * scaling_factor
                        if last_time_to_impact is not None and local_distance < bounce_threshold:
                            if time_to_impact > last_time_to_impact * 1.3:
                                mouse_controller.release(Button.left)
                                released = True
                        if not released:
                                                        #Velocity-band Reaction Delays (tuned At 1440p)                            v = abs(velocity_y)
                        ...
                        #Slow-speed / Emergency Distance Fallbacks            if not released:
                slow_threshold = total_distance * 0.05                  slow_threshold = total_distance * 0.05  #Within 5% Of Green                emergency_threshold = total_distance * 0.025
                if local_distance <= emergency_threshold:
                    mouse_controller.release(Button.left)
                    released = True
                elif local_distance <= slow_threshold and len(white_positions) >= 3:
                                        #Confirm Approach: Latest Distance < Oldest Distance                    recent_dists = [p[1] - green_y for p in white_positions[-3:]]
    ...
    def _execute_shake_click(self, shake_mode):
        ...
        """
                #Get Areas (scale Factor Applied Inside _get_areas)        shake_left_s, shake_top_s, shake_right_s, shake_bottom_s, _, _ = self._get_areas("shake")
        ...
        shake_y = (shake_top_s  + shake_bottom_s) // 2
                #Misc Variables        shake_hex = self.vars["shake_color"]
        ...
        try:
            ...
            failsafe = 80
            shake_clicks = 1
                #Initialize Attempts And Stop Event        attempts = 0
        stop_event = self._start_capture(scan_delay)
        while self.macro_running and attempts < failsafe:
                        #Grab Full Screen Then Crop            if not self._cap_event.wait(timeout=0.5):
                continue
            with self._cap_lock:
                frame = self._cap_frame
                self._cap_event.clear()
            ...
            if shake_area is None or shake_area.size == 0:
                ...
                continue
                        #1. Look For Shake Pixel            if shake_mode == "Pixel":
            ...
            if shake_pixel:
                ...
                self._click_at(screen_x, screen_y, shake_clicks)
                        #2. Fish Detection — Friend Area (green Gone = Minigame Started)            detected = False
            while detected == False and self.macro_running:
                ...
                if not friend_x:
                    ...
                    break
                        #3. Fish Detected → Enter Minigame            if detected == True:
                self.set_status("Entering Minigame")
                mouse_controller.press(Button.left)
                time.sleep(0.003)
                mouse_controller.release(Button.left)
                return                  return  #Exit Shake Cleanly
    ...
    def _execute_shake_navigation(self):
        ...
        self.set_status("Shake Mode: Navigation")
                #Get Areas (scale Factor Applied Inside _get_areas)        fish_left_s, fish_top_s, fish_right_s, fish_bottom_s, _, _         = self._get_areas("fish")
        friend_left_s, friend_top_s, friend_right_s, friend_bottom_s, _, _ = self._get_areas("friend")
                #Misc Variables        scan_delay = float(self.vars["shake_scan_delay"])
        friend_color = self.vars["friends_color"]
        friend_tol = int(self.vars["friends_tolerance"])
        try:
            tolerance = int(self.vars["shake_tolerance"])
        ...
        stop_event = self._start_capture(scan_delay)
        while self.macro_running and attempts < failsafe:
                        #1. Navigation Shake (Enter Key)            keyboard_controller.press(Key.enter)
            time.sleep(0.03)
            ...
            while detected == False and self.macro_running:
                ...
                if frame is None:
                    stop_event.set()
                                        #Print("Finished (no Frame)")                    return
                ...
                time.sleep(0.003)
                mouse_controller.release(Button.left)
                                #Print("Finished (fish Detected)")                return                  return  #Exit Shake Cleanly

            attempts += 1
            time.sleep(scan_delay)
    def _enter_minigame_tranquility(self):
                #Get Colors        left_color = self.vars["left_color"]
        right_color = self.vars["right_color"]
        arrow_color = self.vars["arrow_color"]
        fish_color = self.vars["fish_color"]
                #Get Tolerance        left_tolerance = int(self.vars["left_tolerance"])
        ...
        fish_tolerance = int(self.vars["fish_tolerance"])
                #Get Misc Variables        target = float(self.vars["tranquility_note_ratio"])         target = float(self.vars["tranquility_note_ratio"]) #Changed From 0.7 To Tranquility_note_ratio (using Float)        tranquility_mode = self.vars["tranquility_mode"]
        scan_delay = float(self.vars["minigame_scan_delay"])
        restart_delay = float(self.vars["restart_delay"])
        friend_color = self.vars["friends_color"]
        friend_tol = int(self.vars["friends_tolerance"])
                #Get Areas        shake_left, shake_top, shake_right, shake_bottom, _, shake_height = self._get_areas("shake")
        ...
        while self.macro_running:
            ...
            self._set_fish_overlay_mode("tranquility")
                        #Friend Detection Area            friend_img = frame[friend_top:friend_bottom, friend_left:friend_right]
                        #Step 2: Find Last Pixels In SHAKE And FISH Area            shake_img = frame[shake_top:shake_bottom, shake_left:shake_right]
            ...
            fish_fish_pixel = self._find_last_pixel(fish_img, fish_color, fish_tolerance)
                        #Step 3: Restart Method — Friend Area (green Present = Minigame Ended)            friend_x = self._find_color_center(friend_img, friend_color, friend_tol)
            if friend_x is not None:
                ...
                self._set_fish_overlay_mode("idle")
                return
                        #Step 4: Determine Which Pixel To Use For Each Note Based On Availability                        #Priority: SHAKE Area Pixel If Exists, Otherwise FISH Area Pixel (fixed)            left_pixel = shake_left_pixel[1] if shake_left_pixel is not None else fish_left_pixel[1]
            ...
            fish_pixel = shake_fish_pixel[1] if shake_fish_pixel is not None else fish_fish_pixel[1]
                        #Step 5: Convert To Ratios Using Total_height (fish_bottom - Shake_top)            crop = fish_bottom - shake_top
            ...
            fish_ratio = fish_pixel / crop if not fish_pixel == None else 0
                        #Step 6: Draw                        #You'll Need To Adjust The Drawing Logic Based On Which Pixel Was Used                        #This Is A Placeholder - You May Want To Draw Both Or The Active One            overlay_center_x = fish_height / 2
            note_height = 0.1
            for ratio, color in (
                ...
                self.fish_overlay.draw(
                    ...
                    bar_y2=min(1.0, ratio + note_height / 2)
                        ...
                        #Step 7: Compare Note Ratios To User Given Target (based On Tranquility Mode)            if tranquility_mode == "Steady" or tranquility_mode == "steady":
    ...
    def _enter_minigame(self):
                #Get All 3 Areas        shake_left, shake_top, shake_right, shake_bottom, _, _ = self._get_areas("shake")
        ...
        self._set_fish_overlay_mode("fishing")
                #Load Values From Gui        arrow_hex = self.vars["arrow_color"]
        ...
        last_line_seen_time = time.perf_counter()
                #Hold And Release Mouse        def hold_mouse():
            nonlocal mouse_down
            if not mouse_down:
                self.hold_mouse()
                                #Keyboard_Controller.Press(Key.Space)                mouse_down = True
        def release_mouse():
            nonlocal mouse_down
            if mouse_down:
                self.release_mouse()
        ...
        while self.macro_running:
            ...
            friend_img = frame[friend_top:friend_bottom, friend_left:friend_right]
                        #Cv2.imwrite("screenshot.png", Frame)            if lock_cursor == "on":             if lock_cursor == "on": #Lock Cursor If Enabled                mouse_controller.position = (shake_x, shake_y)
                        #Step 2: Detection            if fishing_mode == "Line":
            ...
            if track_notes == "on":
                ...
                note_box_pos = None
                        #Convert Fish X From Tuple To Int            if fish_x is None:
                pass
            ...
            if fishing_mode == "Line":
                ...
                if line_has_full_detection:
                    ...
                    if (left_x is None or right_x is None) and self._last_bar_left_x is not None and self._last_bar_right_x is not None:
                        ...
                        #Step 3: Calculations            self.fish_overlay.clear()
            any_bar_detected_this_frame = left_x is not None and right_x is not None             any_bar_detected_this_frame = left_x is not None and right_x is not None #Check 1            if any_bar_detected_this_frame:
                detection_source = 0
            else:
                if arrow_method == 2:
                    bar_center, left_x, right_x = self._update_arrow_box_estimation(arrow_indicator_x, any_bar_detected_this_frame, fish_width)
                else:
                                        #This Ensures Rods With 1 Arrow Can Be Tracked Normally                    bar_center = self._find_color_cluster(img, arrow_hex, arrow_tol, 10)
                    try:
                ...
                any_bar_detected_this_frame = True                 any_bar_detected_this_frame = True #Check 2                detection_source = 1
            if any_bar_detected_this_frame and not (left_x == None or right_x == None):             if any_bar_detected_this_frame and not (left_x == None or right_x == None): #Bar Or Arrows Found                bar_size = abs(right_x - left_x)
                bar_center = (left_x + bar_size // 2) + fish_left                 bar_center = (left_x + bar_size // 2) + fish_left #Add Fish Left Here                left_deadzone = bar_size * bar_ratio
            ...
            if detection_source == 0:
                ...
                self._last_bar_right_x = right_x
                                #Completed: Cache Real Box Size                if left_x is not None and right_x is not None:
                    bar_size = abs(right_x - left_x)
                    if bar_size > 0:
                        self.last_cached_box_length = bar_size
                                                #Sync Arrow Estimation Immediately                        self.estimated_box_length = bar_size
                ...
                if self.last_fish_x is not None and abs(fish_x - self.last_fish_x) > 200:
                                        ...
                                        #Accept This Frame And Update Cache                    self.last_fish_x = fish_x
                self.last_fish_x = fish_x
                        #Step 4: Restart Method — Friend Area (green Present = Minigame Ended)            friend_x = self._find_color_center(friend_img, friend_color, friend_tol)
            if friend_x is not None:
                release_mouse()
                time.sleep(restart_delay)
                self._set_fish_overlay_mode("idle")
            ...
            if left_x == None or right_x == None:
                left_x = self._last_bar_left_x
                right_x = self._last_bar_right_x
                        #Position Bar Based On State            if not mouse_down:
                right_x = left_x + bar_size if not left_x == None else None
            else:
                left_x = right_x - bar_size if not right_x == None else None
                        #Step 5: Apply Max Left/Right Calculations            if any_bar_detected_this_frame and bar_center is not None:             if any_bar_detected_this_frame and bar_center is not None: #Bar Found                if note_box_pos is not None:
                                        #Direct Mapping (Already In Fish Space)                    note_screen_x = note_box_pos[0] + fish_left
                ...
                if note_box_pos is not None and track_notes == "on":
                    ...
                    pass
                                #Compute Bar Left And Bar Right (Screen Coords)                bar_left_screen  = left_x  + fish_left if not left_x == None else None
                bar_right_screen = right_x + fish_left if not right_x == None else None
                                #Check Max Left And Max Right                if fish_x == None:
                    fish_x = 0
                if max_left and fish_x <= max_left:                 if max_left and fish_x <= max_left: #Max Left And Right Check (Inside Bar)                    controller_mode = 3
            ...
            if self._is_fish_overlay_enabled():
                ...
                self.fish_overlay.draw(
                    ...
                    color="red", canvas_offset=fish_left
                        ...
                        #Step 7: Controller            error = round(fish_x - bar_center) if bar_center is not None and fish_x is not None else 0
            if controller_mode == 0 and bar_center is not None:
                control = self._pid_control(error, bar_center)
                                #Map PID Output To Mouse Clicks Using Hysteresis To Avoid Jitter/Oscillation                                #Stabilize Deadzone Checker                if control > 0:
                    hold_mouse()
                else:
                    release_mouse()
            elif controller_mode == 1 and bar_center is not None:             elif controller_mode == 1 and bar_center is not None: #Simple Tracking                control = fish_x - bar_center
                                #Stabilize Deadzone Checker                if control > 0:
                    hold_mouse()
                else:
                    release_mouse()
    ...
    def stop_macro(self, text="Macro Stupped"):
        ...
        try:
            ...
            pass
#Check If Index.html Existsif not os.path.exists(os.path.join(UI_PATH, "index.html")):
        #Create And Show A Tkinter Messagebox    from tkinter import messagebox
...
""")
    ...
    def on_main_window_closed():
        ...
        try:
                        #Stop Macro Loop            api.macro_running = False
        except:
            pass
        try:
                        #Stop Capture Thread            api._stop_active_capture(join_timeout=1.0)
        except:
            pass
        try:
                        #Close Area Selector If Open            if hasattr(api, "area_selector") and api.area_selector:
                api.area_selector.close()
        except:
            pass
        try:
                        #Close Eyedropper If Open            if hasattr(api, "eyedropper") and api.eyedropper:
                api.eyedropper.close()
        except:
            pass
        try:
                        #Close Fish Overlay If Open            if hasattr(api, "fish_overlay") and api.fish_overlay:
                api.fish_overlay.close()
        except:
            pass
        try:
                        #Stop Pynput Listener            if hasattr(api, "key_listener") and api.key_listener:
                api.key_listener.stop()
        except:
            pass
        try:
                        #Close MSS Instance For Current Thread            if hasattr(api._thread_local, "sct"):
                api._thread_local.sct.close()
        except:
            pass
        #Attach Shutdown Handler    window.events.closed += on_main_window_closed
