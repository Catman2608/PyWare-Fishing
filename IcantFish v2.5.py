...
class App(CTk):
    ...
    def build_utilities_tab(self, parent):
        ...
        use_sundial_mode_when_cb = CTkComboBox(auto_totem, values=["Enabled", "Disabled"], 
                               variable=use_sundial_mode_when_var, command=lambda v: self.set_status(f"Macro now uses sundial totem when {v}")
                               )
        use_sundial_mode_when_cb.grid(row=4, column=1, padx=12, pady=10, sticky="w")
        self.comboboxes["use_sundial_mode_when"] = use_sundial_mode_when_cb
    ...
    def execute_totem(self, cycle, shake_x, shake_y):
        ...
        if required_cycle > 0 and cycle % required_cycle == 0:
            ...
            if not day_night == None and not use_sundial_mode_when == "Disabled":
